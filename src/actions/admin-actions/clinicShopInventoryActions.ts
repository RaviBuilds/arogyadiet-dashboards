...
/**
 * The RPC exceptions raised by `clinic_shop_stock_in`,
 * `clinic_shop_apply_sale`, and `set_clinic_product_visibility` carry a stable
 * prefix (design.md "Message mapping") so this layer can translate a Postgres
 * exception into the requirement's specified wording without string-sniffing
 * internals. The detail text after the prefix already names the offending
 * product(s) and quantity/quantities — generated by the RPC itself — so it is
 * appended rather than re-derived.
 *
 * Order matters only in that every prefix is distinct and matched by
 * substring, so declaration order is not significant.
 */
 ...
 */
function mapClinicStockRpcError(rawMessage: unknown): string {
