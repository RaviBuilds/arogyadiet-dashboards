...
export function PauseClient({
  ...
  // Calculate dynamic pause usage
  const newlyPaused = pausedDates.filter(
    (d) => !initialPausedDates.includes(d),
  ).length;
  const newlyUnpaused = initialPausedDates.filter(
    (d: string) => !pausedDates.includes(d),
  ).length;
  ...
  const handleTogglePause = (dateStr: string) => {
    setPausedDates((prev) => {
      if (prev.includes(dateStr)) {
        ...
        if (isLimitReached) return prev; // Security block
        return [...prev, dateStr]; // Pause
      }
    });
  };
  ...
  const handleSave = async () => {
    ...
    // 3. REMOVED netPauseChange from the arguments
    const result = await bulkUpdatePausePreferencesAction(
      subscriptionId,
      updates,
    );
  ...
  const hasChanges =
    netPauseChange !== 0 || newlyPaused > 0 || newlyUnpaused > 0;
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 border rounded-xl shadow-sm sticky top-[60px] z-10">
        ...
        <Button
          ...
          className="w-full sm:w-auto font-bold transition-all bg-blue-600 hover:bg-blue-700"
        >
          {isSaving ? (
            <>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            >
              You have used <strong>{currentPausesUsed}</strong> of{" "}
            ...
            >
              ...
              >
                ...
                {daysInMonth.map((date, index) => {
                  ...
                  // Disable if locked, OR if they are trying to click an active day but the limit is reached
                  const isDisabled =
                    isLockedOut || (!isPaused && isLimitReached);
