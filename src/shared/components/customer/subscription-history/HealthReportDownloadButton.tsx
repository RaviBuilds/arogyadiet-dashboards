 ...
 *
 * Triggers a PDF download from /api/meal-health-report/[subscriptionId].
 * Shows a loading indicator during generation and surfaces error states.
 * Disabled for PENDING subscriptions (no logging window yet).
 */
...
interface HealthReportDownloadButtonProps {
  subscriptionId: string;
  status: string;
}

export function HealthReportDownloadButton({
  subscriptionId,
  status,
}: HealthReportDownloadButtonProps) {
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const isDisabled = status.toUpperCase() === "PENDING";
  ...
  async function handleDownload() {
    ...
    try {
      const response = await fetch(`/api/meal-health-report/${subscriptionId}`);
      ...
      link.href = url;
      link.download = `health-report-${subscriptionId}.pdf`;
