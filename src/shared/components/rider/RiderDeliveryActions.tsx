...
// Lazy-load the modals. They pull in Dialog/Select/Textarea/Checkbox primitives
// that aren't needed for the initial route screen render. They're only fetched
// the first time the rider opens the corresponding dialog, cutting the initial
// JS execution cost on the (frequently visited) delivery screen.
const ChecklistModal = dynamic(
  () => import("./ChecklistModal").then((m) => ({ default: m.ChecklistModal })),
  { ssr: false },
);
const FailedDeliveryModal = dynamic(
  () =>
    import("./FailedDeliveryModal").then((m) => ({
      default: m.FailedDeliveryModal,
    })),
  { ssr: false },
);
