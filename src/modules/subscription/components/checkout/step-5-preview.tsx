...
import {
  ChevronLeft, MapPin, Calendar, Utensils,
  ShieldCheck, Loader2, Receipt, Tag, X,
} from "lucide-react";
...
export function OrderPreview({ data, plans, onBack }: any) {
  ...
  useEffect(() => {
    async function fetchProfile() {
      ...
      if (appUser) {
        const { data: profile } = await supabase.from("customer_profiles").select("id").eq("user_id", appUser.id).maybeSingle();
  ...
  useEffect(() => {
    async function fetchAddress() {
      if (!data.addressId) { setIsLoadingAddress(false); return; }
      const supabase = createClient();
      const { data: addr } = await supabase.from("addresses").select("*").eq("id", data.addressId).single();
  ...
  const handleApplyCoupon = async () => {
    ...
    const res = await validateCouponAction(couponInput.trim(), customerProfileId, baseDuration);
  ...
  const handlePayment = async () => {
    if (!customerProfileId) { alert("Customer profile not loaded. Please refresh the page."); return; }
    ...
    try {
      ...
      const orderRes = await createRazorpayOrderAction(data.planId, baseDuration, appliedCoupon?.code, customerProfileId);
      if (!orderRes.success || !orderRes.order) throw new Error("Could not create order");
      ...
      const options = {
        ...
        handler: async function (response: any) {
          ...
          const verifyRes = await verifyAndActivateSubscriptionAction(response, finalCheckoutData, appliedCoupon?.code);
  ...
  return (
    <div className="space-y-6 sm:space-y-10 animate-in fade-in slide-in-from-right-4 max-w-full overflow-hidden">
      <section>
        ...
        <div className="lg:col-span-7 space-y-6">
          <Card className="border sm:border-2 overflow-hidden shadow-sm">
            <div className="bg-zinc-50 border-b px-4 sm:px-6 py-3 sm:py-4 flex items-center gap-2">
              ...
              <h3 className="font-bold text-base sm:text-lg text-zinc-800">Subscription Summary</h3>
            </div>
            ...
            <CardContent className="p-0">
              <div className="p-4 sm:p-6 flex items-start gap-3 sm:gap-4">
                ...
                <div className="min-w-0 flex-1">
                  <h4 className="font-bold text-base sm:text-lg text-zinc-900 truncate">{planName}</h4>
                  <div className="flex flex-col mt-1 sm:mt-2 text-xs sm:text-sm text-zinc-600 space-y-1">
                    <p>Starts: <strong className="text-zinc-900">{data.startDate ? format(new Date(data.startDate), "MMM do, yyyy") : "TBD"}</strong></p>
                    <p>Ends: <strong className="text-zinc-900">{endDate ? format(endDate, "MMM do, yyyy") : "TBD"}</strong></p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 sm:p-6 flex items-start gap-3 sm:gap-4">
                ...
                <div className="min-w-0 flex-1">
                  <h4 className="font-bold text-base sm:text-lg text-zinc-900">Meal Schedule</h4>
                  <p className="text-xs sm:text-sm text-zinc-600 mt-1">Base Plan: <strong className="text-zinc-900">{data.foodType}</strong></p>
                  
                  <div className="mt-3 bg-zinc-50 border rounded-lg p-3 w-full">
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mb-2">
                      <span className="text-xs sm:text-sm font-semibold text-zinc-700">Pause Credits Used</span>
                      <span className="text-xs font-bold bg-white border px-2 py-1 rounded w-fit">{pausesUsed} of {maxPauses}</span>
                    </div>
                    {pausesUsed > 0 ? (
                      <div className="text-[11px] sm:text-xs text-zinc-600 leading-relaxed break-words">
                        <span className="font-medium">Paused Dates:</span> {pausedDatesArray.sort().map((d: string) => format(new Date(d), "MMM do")).join(", ")}
                      </div>
                    ) : (
                      <p className="text-[11px] sm:text-xs text-zinc-500">No dates paused.</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 sm:p-6 flex items-start gap-3 sm:gap-4">
                ...
                <div className="min-w-0 flex-1">
                  <h4 className="font-bold text-base sm:text-lg text-zinc-900">Delivery Address</h4>
                  {isLoadingAddress ? (
                    ...
                    <div className="mt-2 text-xs sm:text-sm text-zinc-600 leading-relaxed break-words">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-bold text-zinc-900 truncate">{deliveryAddress.tag}</span>
                      </div>
                      <p>{deliveryAddress.street_1}, {deliveryAddress.city}</p>
                    </div>
                  ) : (
                    <p className="text-xs sm:text-sm text-red-500 mt-2">Error loading address details.</p>
                  )}
                </div>
              </div>
            </CardContent>
        ...
        <div className="lg:col-span-5 space-y-6">
          <Card className="border sm:border-2 overflow-hidden shadow-sm">
            <CardContent className="p-4 sm:p-6">
              <div className="flex items-center gap-2 mb-3 sm:mb-4">
                ...
                <h3 className="font-bold text-base sm:text-lg text-zinc-800">Apply Coupon</h3>
              </div>
                ...
                <div className="space-y-2 sm:space-y-3">
                  <div className="flex gap-2">
                    ...
                    <Button 
                      ...
                      disabled={isApplyingCoupon || !couponInput || isLoadingProfile}
                    >
                      {isApplyingCoupon ? <Loader2 className="h-4 w-4 animate-spin" /> : "Apply"}
                    </Button>
                  </div>
                  {couponError && <p className="text-[11px] sm:text-xs text-red-500 font-medium">{couponError}</p>}
                </div>
              ) : (
                <div className="bg-green-50 border border-green-200 rounded-lg p-3 flex justify-between items-center">
                  <div className="min-w-0">
                    <p className="text-xs sm:text-sm font-bold text-green-800 flex items-center gap-1.5 truncate">
                      <ShieldCheck className="h-3 w-3 sm:h-4 sm:w-4 shrink-0" /> Applied!
                    </p>
                    <p className="text-[10px] sm:text-xs text-green-700 mt-0.5 truncate">
                      Code: <strong>{appliedCoupon.code}</strong>
                    </p>
                  </div>
                  <Button variant="ghost" size="icon" onClick={handleRemoveCoupon} className="text-green-800 hover:bg-green-100 shrink-0 h-8 w-8">
          ...
          <Card className="border sm:border-2 overflow-hidden shadow-sm">
            <div className="bg-zinc-50 border-b px-4 sm:px-6 py-3 sm:py-4 flex items-center gap-2">
              ...
              <h3 className="font-bold text-base sm:text-lg text-zinc-800">Price Details</h3>
            </div>
            
            <CardContent className="p-4 sm:p-6 space-y-3 sm:space-y-4">
              <div className="flex justify-between text-xs sm:text-sm text-zinc-600">
                <span>Base Price</span>
                <span className="font-medium text-zinc-900">₹{originalBasePrice.toLocaleString("en-IN")}</span>
              </div>
              ...
              <div className="flex justify-between text-xs sm:text-sm text-zinc-600">
                ...
                <span className="font-medium text-zinc-900">₹{gst.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
              </div>
              ...
              <div className="flex justify-between items-center">
                <span className="font-bold text-base sm:text-lg text-zinc-900">Total</span>
              ...
              <div className="pt-2 sm:pt-4">
                <Button 
                  size="lg" 
                  disabled={isProcessing || isLoadingAddress || isLoadingProfile}
                  onClick={handlePayment}
                  className="w-full text-base sm:text-lg font-bold h-12 sm:h-14 bg-primary hover:bg-primary/90 text-white transition-all active:scale-95 disabled:opacity-70"
                >
                  {isProcessing ? (
      ...
      <div className="pt-4 sm:pt-8 border-t flex flex-col-reverse sm:flex-row justify-between items-center gap-4 mt-8 pb-6">
        <Button variant="ghost" onClick={onBack} disabled={isProcessing} className="w-full sm:w-auto gap-2 hover:bg-zinc-100 text-sm">
