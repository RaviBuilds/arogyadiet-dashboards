...
### 6.7 Franchise RBAC (Role‑Based Access Control) — recent addition

The franchise portal used to be gated by a single boolean: "is this session `FRANCHISE_ADMIN` with a non‑suspended `franchise_id`?" That has been replaced with a **per‑Operations‑Group Manage/View model**, reusing the same primitives the admin portal's `admin_access_level`/`admin_operations_access` system already used (§5.1), rather than inventing a parallel schema. **No new database table was created** — the only schema change is one additive column, `users.admin_operations_access JSONB` (`scripts/add-admin-operations-access-to-users.sql`, shape `{ "<group>": "manage"|"view" }`, with a one‑time backfill setting every existing `ADMIN` to `admin_access_level = 'inventory_operations'`). Franchise users are just rows in the same `users` table with `role = FRANCHISE_ADMIN` and `franchise_id` set.

**Shared primitives** (`src/lib/auth/adminAccessCore.ts`, edge‑safe, no Supabase/Next imports — used by both middleware and layouts):
- `OPERATIONS_GROUPS` (6 groups: `customers`, `subscriptions`, `riders`, `operations`, `franchises`, `shop_products`) for admin, and a **franchise‑specific subset** `FRANCHISE_OPERATIONS_GROUPS` (5 groups — `franchises` deliberately excluded, since that group governs the cross‑network franchise list, a Core‑Business‑only concern; a hand‑crafted request granting it is also rejected server‑side by `validateFranchiseOperationsAccessInput`).
- `PERMISSION_LEVELS = ["manage", "view"]`; `hasGroupAccess()` / `canManageGroup()` do the read‑vs‑write check per group.
- `classifyAdminPath()` / `isPortalPathAllowed()` classify a URL into an Operations_Group by longest‑prefix match; `toCanonicalPath()` rewrites `/franchise/*` paths onto the admin's canonical `/admin/*` classification space (with explicit aliases, e.g. `/franchise/shop-products` → `/admin/kitchen-shop`, `/franchise/orders|disputes|reports` → `/admin/operations`), so the franchise portal's differently‑named routes are gated by the exact same group logic as admin without a second classification table.
- `DIETITIAN_ACCESS_LEVEL` remains a special no‑groups allow‑list level (own prefix list), used identically for Core and Franchise dietitians.

**Server‑only resolver/guards** (`src/lib/auth/adminAccess.ts`):
- `resolveFranchiseAccessContext()` — resolves the caller's identity, applies the **Franchise_Owner override** (the franchise's `owner_user_id` always resolves to full `inventory_operations` access, derived on the fly — never stored as a row value), and rejects a suspended franchise.
- `assertFranchiseGroupManage(group)` / `checkFranchiseGroupManage(group)` — write‑path guards used inside franchise Server Actions. Kept deliberately separate from the admin's `assertGroupManage`, because some shared admin actions are directly importable from the franchise bundle (e.g. parts of `Customer360Dashboard`'s fallback path) and carry no franchise‑ownership check of their own — this guard is what actually stops a cross‑tenant write on those call sites.
- `checkFranchiseCustomersRead()` / `guardFranchiseCustomersWorkspace()` — read‑path page guard for the customers workspace; additionally admits **Dietitians**, who hold no Operations_Group at all (a plain `hasGroupAccess` check would infinitely redirect a dietitian whose landing route is itself `/customers`).

**Master‑side provisioning UI** — this is the "some code changes" visible at `master.arogyadiet.com/hierarchy`: `FranchiseUsersPanel.tsx`, a dialog opened from a franchise's row in the Hierarchy tree (`src/app/master/(main)/hierarchy/page.tsx` → `HierarchyTree`). It lists every `users` row for that franchise (`listFranchiseUsers`) and offers:
- **Create Franchise User** — name/email/mobile/password + an **Access Level** select (`FRANCHISE_USER_ACCESS_LEVELS` = `inventory` / `operations` / `inventory_operations`, no `dietitian` option here) + an `OperationsGroupConfig` manage/view matrix, shown only when level = `operations`, scoped to the 5 `FRANCHISE_OPERATIONS_GROUPS`.
- **Create/Edit Dietitian** — a separate action; the dietitian's clinic is auto‑resolved server‑side and the control is disabled until the franchise actually has a wired clinic.
- **Edit / toggle‑active / delete** per franchise user — the Owner is protected from deactivation/deletion and is shown as a derived "full access" badge rather than a stored value.
- Backed entirely by `src/actions/master-actions/franchiseUserActions.ts` (`listFranchiseUsers`, `createFranchiseUser`, `createFranchiseDietitian`, `updateFranchiseUser`, `toggleFranchiseUserActive`, `deleteFranchiseUser`), every call scoped by `franchiseId` and reusing the same validation primitives as the core admin's user management.

**Middleware enforcement** (`src/middleware.ts`): a `FRANCHISE_ADMIN` hitting `admin.*` or `master.*` is bounced back to `franchies.*` (flag‑gated); only `FRANCHISE_ADMIN` may reach `franchies.*` at all; once there, the franchise's `franchise_id`/suspended‑status is checked, and the **same** `isPortalPathAllowed()` group gate the admin portal uses is applied on the `/franchise` path base — with the Franchise_Owner treated as `inventory_operations`.

**Franchise `/customers` page (`franchies.arogyadiet.com/customers`) — the code change referenced by the user:**
- `src/app/franchise/(main)/customers/page.tsx` now opens with a single `guardFranchiseCustomersWorkspace()` call, replacing an older ad‑hoc `x-franchise-id` cookie read plus a separate `admin_access_level` query — neither of which had checked the caller's Operations_Group at all; the page previously relied entirely on the middleware's route‑level gate. The guard returns `{ franchiseId, canManage, isDietitian }`, with `franchiseId` resolved from the caller's own `users` row (not the cookie).
- Customers are still fetched with the service‑role client (`createAdminClient()`), hard‑filtered `.eq("franchise_id", franchiseId)` — RLS is bypassed here, so tenant isolation at this call site remains app‑layer only (the same pattern already documented for the admin portal in §5.1/§5.3).
- **Dietitian read‑scope narrowing**: `scopeFranchiseCustomersForDietitian()` (`src/lib/dietitian/franchiseCustomerScope.ts`) restricts a Dietitian's result set to only customers where `dietitian_id` matches them (not the whole franchise tenant), failing **closed** (empty list) if the dietitian context can't be resolved. `scripts/allow-multiple-franchise-dietitians.sql` later loosened the underlying DB predicate to permit multiple dietitians per franchise while keeping each dietitian's own customer isolation intact.
- `canManage` / `isDietitian` are threaded down into `FranchiseCustomerDashboard` (the customer‑dashboard UI itself is a separate, later enhancement — see §9), and further into leaf components like `KitCustomerSection.tsx` and `DietitianAssignmentSelector.tsx`, which branch on `"view"` vs `"manage"` to hide mutating controls — the write‑suppression half of the enforcement, reaching into shared UI rather than stopping at the page/action layer.

**Caveats worth carrying forward:**
- The `franchise-scoped-access` work (naming convention seen throughout the code comments/tests above) has **no corresponding `.kiro/specs/franchise-scoped-access/` folder** in the repo — either it was executed without a formal spec, or its spec files were removed after completion. Task‑completion status for this specific feature could not be verified against a spec checklist the way most other features in this document were.
- No RLS policies exist yet for the RBAC columns/groups themselves (not needed — it's an application permission model, not row data), but it's worth noting the pre‑existing franchise‑hierarchy RLS policies (`create-franchise-hierarchy-rls-policies.sql`) remain created‑but‑**not‑enabled**, consistent with the RLS‑bypass pattern the franchise customers query above also relies on.
...
| Franchise RBAC (per‑Operations‑Group Manage/View access for franchise users) | code confirmed live: `adminAccessCore.ts` (`FRANCHISE_OPERATIONS_GROUPS`), `adminAccess.ts` (`resolveFranchiseAccessContext`, `assertFranchiseGroupManage`, `guardFranchiseCustomersWorkspace`), master `FranchiseUsersPanel.tsx` + `franchiseUserActions.ts`, `franchise/(main)/customers/page.tsx` guard rewrite; see §6.7 | High for the mechanism — but referenced everywhere as "franchise‑scoped‑access Task N" with **no matching `.kiro/specs/franchise-scoped-access/` folder present**, so its own checklist could not be verified |
